"""
app.py - NexaCV Flask Application
...
app = Flask(__name__)
app.secret_key = os.getenv("SECRET_KEY", "nexacv-secret-2024-xK9mP3qL")
app.config["MAX_CONTENT_LENGTH"] = 10 * 1024 * 1024
app.config["SESSION_PERMANENT"] = False

# ── File-based store for analysis data ────────────────────────────────────────
...
def upload():
    ...
    return render_template("upload.html",
                           username=session.get("username"), error=error)
...
def run():
    ...
    if not data:
        return redirect(url_for("upload"))
    try:
        print("[run] Step 1: Extracting skills...")
        ...
        return render_template("error.html",
                               username=session.get("username"), error=str(e))
...
def history():
    ...
    return render_template("history.html",
                           username=session.get("username"), records=records)
